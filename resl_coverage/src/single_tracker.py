...
def twist_callback(msg):
    ...
    noise = np.dot(R, np.random.random(4))
    z[2] = msg.twist.linear.x + (noise[0] * 0.01)
    z[3] = msg.twist.linear.y + (noise[1] * 0.01)

    u[0] = msg.twist.linear.x
    u[1] = msg.twist.linear.y
    if not set_est[1]:
        x[2] = msg.twist.linear.x
        x[3] = msg.twist.linear.y
        set_est[1] = True
